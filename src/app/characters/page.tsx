  ...
  return (
    <div className="flex flex-col min-h-full">
      ...
      <main className="flex-1 max-w-2xl mx-auto w-full px-4 py-8 space-y-8">
        ...
        <section>
          <h2 className="text-lg font-semibold mb-3">新しいキャラクターを作る</h2>
        ...
        <section>
          <h2 className="text-lg font-semibold mb-3">キャラクター一覧</h2>
          {characters.length === 0 ? (
            <p className="text-sm text-gray-500">キャラクターがいません</p>
          ) : (
            <ul className="space-y-2">
              {characters.map((c) => (
                <li key={c.id} className="flex items-center justify-between border border-gray-200 rounded px-4 py-3">
                  {editId === c.id ? (
